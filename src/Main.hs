...
data Command
    = ExprCommand Expr        -- Represents a mathematical expression
    | MemorySet String Double -- Command to set a memory value
    | MemoryPrint             -- Command to print memory
    deriving (Show)
...
repl state = do
    ...
    case parseCommand input of
        ...
        Right (ExprCommand expr) -> do
            ...
            if isNaN result
                then putStrLn "Error: Invalid operation (e.g., square root of a negative number)."
            else if isInfinite result
                then putStrLn "Warning: Division by zero resulted in Infinity."
            else print result
            repl newState
